...
/********************************************************
 ARDUINO SMARTGPU2 LIBRARY VERSION V3.0 - SUPPORTS ARDUINO DUE BOARDS
 IMPORTANT : This library is created for the Arduino 1.5.2 Software IDE
********************************************************/
...
//Arduino Exclusive - Hardware Configuration
void SMARTGPU2::init(){          //configure the arduino board for SMARTGPU2 board
	analogReference(ARDUINOREFERENCE);   //set analog reference as EXTERNAL to avoid damage to arduino boards
	setBaud(9600);
	pinMode(RESET, OUTPUT);      // set pin as output
	digitalWrite(RESET, HIGH);   // set the pin to 5v to end reset	
}
