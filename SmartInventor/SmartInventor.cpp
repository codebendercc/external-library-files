...
{
	remoconUseTV = 0;
	if(dcUse == 0 || remoconUseTV == 0 || remoconUseRF == 0)
	{
		#if defined(TIMER)
			TIMSK |= (0<<TOIE2);  
		#elif defined(TIMER0)
...
//		Button_center = 512;
		...
		Button_funcr = (unsigned char) Rf_KEY_5;
		Button_funcl = (unsigned char) Rf_KEY_6;
	}
...
{
	word data = 0xFFFF;
	int num = _SERIAL_.available();	
	
	for(int i=0; i<num; i++)
	{		
		int input = _SERIAL_.read();	 // this is 'int' to handle -1 when no data		
